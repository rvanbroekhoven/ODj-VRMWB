document.addEventListener('DOMContentLoaded', () => {
    ...
    function tick() {
        ...
        if (dateStr !== _lastDate) { 
            const bbdate = document.getElementById('bbdate');
            if (bbdate) bbdate.textContent = dateStr; 
            _lastDate = dateStr; 
        }
        if (timeStr !== _lastTime) { 
            const bbtime = document.getElementById('bbtime');
            if (bbtime) bbtime.textContent = timeStr; 
            _lastTime = timeStr; 
        }
    }
    ...
    function updateCenterNavigation() {
        const btnTodayText = document.getElementById('bb-date-display');
    ...
    function getGlassRowsKISHTML(title, incidents) {
        let html = `<h1 class="slide-title">${title}</h1><div style="width:100%;">`;
        html += `<div style="display:flex; gap:20px; font-weight:700; color:var(--text-muted); padding:0 30px 10px; font-size:16px; text-transform:uppercase;"><div style="flex:1;">Melding</div><div style="flex:1;">Locatie</div><div style="flex:1;">Datum</div><div style="flex:1;">Details</div><div style="flex:0 0 36px;"></div></div>`;
        incidents.forEach(inc => {
            html += `
                <div class="kis-glass-row">
                    ...
                    ${inc.infoText ? `<div class="kis-info-trigger" data-title="${inc.infoTitle}" data-text="${inc.infoText}">i</div>` : '<div style="flex:0 0 36px;"></div>'}
                </div>`;
        });
        html += `</div>`;
        return html;
    ...
    if(btnMagicWand) {
        btnMagicWand.addEventListener('click', () => {
            ...
            const blocksToAdd = ['ploeg-indeling', 'alarmen', 'mobiliteit', 'onderhoud', 'algemeen', 'evenementen'];
            
            blocksToAdd.forEach(id => {
                const card = leftList.querySelector(`[data-id="${id}"]`);
                if(card) {
                    card.classList.remove('theme-card-light');
